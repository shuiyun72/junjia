<template>
	<view class="add_address">
		<uni-nav-bar color="#333333" background-color="#ffffff" :status-bar="true" left-icon="arrowleft" :title="pageTitle"
		 @clickLeft="back" />
		<uni-list>
			<sy-list-item title="收货地址"  @click="showMap">
				<template v-slot:right="">
					{{address1}}
				</template>
			</sy-list-item>
			<sy-list-item title="详细地址"  :showArrow="false">
				<template v-slot:right="">
		...
		<uni-list>
			<sy-list-item title="联系人"  :showArrow="false">
				<template v-slot:right="">
					<input type="text" v-model="name" placeholder="联系人姓名" class="addr_input" />
				</template>
			</sy-list-item>
			<sy-list-item title="联系电话"  :showArrow="false">
				<template v-slot:right="">
